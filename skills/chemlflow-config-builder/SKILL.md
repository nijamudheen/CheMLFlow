...
- **Fixed params vs DOE model search**: set concrete `train.model.params` in a single runtime config. Use DOE `model_search.method: grid` or `model_search.method: optuna` when hyperparameter candidates should become scientific parent cases that then fan out across folds. Runtime child-level search is disabled, so keep `train.tuning.method: fixed` and do not set `train.tuning.use_hpo: true`.
