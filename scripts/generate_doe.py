...
def main(argv: list[str] | None = None) -> int:
    ...
    if "total_parent_cases" in summary:
        print(
            "[doe] parents={parents} execution={execution}".format(
                parents=summary["total_parent_cases"],
                execution=summary["total_execution_cases"],
            )
        )
    print(f"[doe] manifest: {result['manifest_path']}")
    if result.get("parent_manifest_path"):
        print(f"[doe] parent_manifest: {result['parent_manifest_path']}")
        ...
        json.dumps(
            {
                ...
                "parent_manifest_path": result.get("parent_manifest_path", ""),
            }
        )
    )
    return 0
